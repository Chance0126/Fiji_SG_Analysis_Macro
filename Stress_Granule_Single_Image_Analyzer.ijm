// ImageJ Macro: Single Image Stress Granule Analyzer
...
#@ String (label = "Close extra windows?", choices={"At Beginning", "At End", "Both", "Neither"}, style="radioButtonHorizontal") cleanup_mode
...
// A. Ensure image is open, clean up
n = nImages;
if (nImages == 0) {
    exit("No image found. Please open an image to analyze.");
}
originalID = getImageID();
if (cleanup_mode == "At Beginning" || cleanup_mode == "Both") {
    selectImage(originalID);
    close("\\Others");
} 
else if (nImages > 1) {
    waitForUser("Warning", "You have " + nImages + " images open.\n\n" +
    "This macro is designed for single-image analysis.\n" +
    "Please ensure the correct image is active before clicking OK.");
    
    if (nImages > 0) originalID = getImageID();
}
...
// Check extension (Modified: 'continue' changed to 'exit')
if (!endsWith(filename, ".czi") && !endsWith(filename, ".tif") && !endsWith(filename, ".tiff")) {
    exit("Error: The active image is not a supported format (.czi, .tif, .tiff).");
}
...
if (analyze_aux) {
    ...
    print("Image " + title + " is not multichannel.");
    close();
    continue;
}

// -----------------------------------------------------------
// 3. ANALYZE NUCLEI USING STARDIST (Net 1)
// -----------------------------------------------------------
selectWindow("DAPI");
...
// -----------------------------------------------------------
// 4. ANALYZE NUCLEI (STARDIST OUTPUT) IN IMAGEJ
// -----------------------------------------------------------
...
// -----------------------------------------------------------
// 10. PRINT SUMMARY DATA IN LOG
// -----------------------------------------------------------
if (cleanup_mode == "At End" || cleanup_mode == "Both") {
...
// 1. Close the previous report if it exists (Fixes the duplicate text issue)
if (isOpen(rt)) {
    selectWindow(rt);
    run("Close");
}

// 2. Build the Report Text (Explicit \n ensures correct formatting)
// We build one long string variable
rep = "";
rep = rep + "==================================================\n";
rep = rep + " ANALYSIS SUMMARY\n";
rep = rep + " Image:    " + filename + "\n";
rep = rep + " Pipeline: " + pipeline + "\n";
rep = rep + " Time:     " + getTime() + "\n";
rep = rep + "==================================================\n\n";

rep = rep + "--- COUNTS & RATIOS ---\n";
rep = rep + " Nuclei Count: .......... " + nuclei_count + "\n";
rep = rep + " Total SG Count: ........ " + sg_count + "\n";
rep = rep + "    > Small SGs: ........ " + count_small + "\n";
rep = rep + "    > Large SGs: ........ " + count_large + "\n\n";
rep = rep + " SGs Per Cell: .......... " + sg_per_cell + "\n";
rep = rep + "    > Small/Cell: ....... " + small_per_cell + "\n";
rep = rep + "    > Large/Cell: ....... " + large_per_cell + "\n\n";
rep = rep + " Positive Cells: ........ " + pct_pos_count + " / " + nuclei_count + "\n";
rep = rep + " % Positive: ............ " + percent_positive + "%\n";
rep = rep + "--------------------------------------------------\n";

rep = rep + "--- SIZE MEASUREMENTS (µm²) ---\n";
rep = rep + " Avg Nucleus Area: ...... " + avg_nucleus_area + "\n";
rep = rep + " Avg Cytoplasm Area: .... " + avg_cyto_area + "\n";
rep = rep + " Avg Nucleus Feret: ..... " + avg_nucleus_feret + "\n\n";
rep = rep + " Total SG Area: ......... " + totalArea_um2 + "\n";
rep = rep + " Avg SG Area (All): ..... " + avgArea_um2 + "\n";
rep = rep + "    > Avg Small Area: ... " + avg_area_small + "\n";
rep = rep + "    > Avg Large Area: ... " + avg_area_large + "\n";
rep = rep + "--------------------------------------------------\n";

rep = rep + "--- INTENSITY (Gray Values) ---\n";
rep = rep + " Mean SG Intensity: ..... " + intMean + "\n";
rep = rep + "    > Small Intensity: .. " + avg_int_small + "\n";
rep = rep + "    > Large Intensity: .. " + avg_int_large + "\n";

if (analyze_aux) {
    rep = rep + "\n--- AUX CHANNEL ---\n";
    rep = rep + " Aux Avg Mean: .......... " + aux_avg_mean + "\n";
    rep = rep + " Aux Avg IntDen: ........ " + aux_avg_intden + "\n";
}
rep = rep + "--------------------------------------------------\n";

rep = rep + "--- MORPHOLOGY & SHAPE ---\n";
rep = rep + " Avg Circularity: ....... " + avg_circ + "\n";
rep = rep + "    > Small: " + avg_circ_small + " | Large: " + avg_circ_large + "\n";
rep = rep + " Avg Solidity: .......... " + avg_solid + "\n";
rep = rep + "    > Small: " + avg_solid_small + " | Large: " + avg_solid_large + "\n";
rep = rep + " Avg Feret (Max Dia): ... " + avg_feret + "\n";
rep = rep + "    > Small: " + avg_feret_small + " | Large: " + avg_feret_large + "\n";
rep = rep + "--------------------------------------------------\n";

rep = rep + "--- VARIANCE & SD ---\n";
rep = rep + " SG Area SD: ............ " + sd_um2 + "\n";
rep = rep + " SG Area Variance: ...... " + variance_um2 + "\n";
rep = rep + "==================================================";

// 3. Create the Window and Print
// We use 'File.name' trick or just print to a fresh editor window
run("Text Window...", "name=["+rt+"] width=65 height=45");
print("[" + rt + "]", rep);
