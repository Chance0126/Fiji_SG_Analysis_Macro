...
// Developed for the Smaldino Lab
...
// Developed by Chance Creviston, written primarily by ChatGPT and Google Gemini
...
if (aux_channel_choose == "None") {
    ...
    if (aux_channel == sg_channel) {
        exit("Error: You selected Channel " + aux_channel + " for both SG and Auxiliary.\nPlease restart and choose different channels.");
    }
}

// C. Set minimum object sizes 
...
waitForUser("Open Image", "1. The composite .czi, .tif, or .tiff image file is opened.");
...
waitForUser("Split Channels", "2. The channels are split and labeled according to user-defined selections\nin the opening menu.");
...
waitForUser("Duplicate DAPI", "3. Duplicate the DAPI image so it can be analyzed without destroying the original.");
// C. Run StarDist
...
waitForUser("Run StarDist", "4. Run StarDist (First Network) to find and outline nuclei.");
...
if (nuclei_count > 0) {
    ...
    pixelArea_um2 = pw * ph;
waitForUser("Get DAPI Dimensions", "5. Select DAPI original and collect dimension data so output is in microns, not pixels.");
...
waitForUser("Loop Through Nuclei", "6. Measure data for every nucleus identified by StarDist");
    ...
    run("Clear Results");
    waitForUser("Save Data to .csv file after each image", "7. Macro saves data of each nucleus after each image, then clears results for next section.");
}
...
roiManager("Fill"); 
roiManager("Reset");
waitForUser("Save Nuclei as White", "8. Create new mask of StarDist nuclei measured white for downstream steps.");
...
waitForUser("Create Image for Cellpose", "9. Cellpose needs nuclei and cytoplasmic markers to find boundaries.");
...
// We use env_type=venv (assuming you fixed the activation script with activate.bat and did not enable "Add to PATH" in miniforge3 installation.)
run("Cellpose SAM...", 
...
run("16-bit"); 
waitForUser("Run Cellpose SAM", "10. Wait for Cellpose (Second Network), collect and rename the labeled output image.");
...
waitForUser("Create Maximum Limit Mask", "11. Expand nuclei to create limits for cells. We will use this to remove\nextraneous cytoplasms detected without nuclei.");

selectWindow("Cellpose_Raw_Labels");
run("Restore Selection"); waitForUser("Add Maximum Mask to CP Output", "12. Add the maximum mask to cellpose output, then...");
setBackgroundColor(0, 0, 0); // Ensure 'Clear' makes pixels 0 (Black)
run("Clear Outside"); 
run("Select None"); 
waitForUser("Clear Extra Cytoplasm", "13. Clear extraneous cytoplasm not detected within range of nuclei.");
...
waitForUser("Loop Through Cells", "14. Add cell areas to the ROI manager.");
...
if (roiManager("count") > 0) {
    selectWindow("SG_Channel"); 
    roiManager("Deselect"); 
    waitForUser("Select SG_Channel", "15. Select the SG channel so cytoplasmic measurements are recorded in the correct image.");
...
waitForUser("Loop Through Cells", "16. Measure data of each cell measured by Cellpose.");
...
waitForUser("Save Cell Data", "17. All cell data is saved to .csv file for cell boundaries after each image.");
...
run("Clear"); 
run("Select None");
waitForUser("Duplicate Cellpose Output", "18. Duplicate for downstream steps.");
...
// Convert to Mask
setThreshold(1, 255);
run("Convert to Mask"); waitForUser("Convert Cellpose Output to Mask", "19. Create a white mask of cells for downstream steps.");
run("Duplicate...", "title=Cell_Territories_Grounded");
run("Duplicate...", "title=Cell_Territories_Grounded_Aux"); 
waitForUser("Duplicate", "20. Duplicate for SG and Aux analysis steps.");
...
// =======================================================
    ...
    waitForUser("Duplicate SG_Channel", "9. Duplicate for classification.");
    ...
    run("Convert to Mask");
    run("Fill Holes");
    rename("LabKit_Raw"); waitForUser("Collect LabKit Output", "10. Collect LabKit (Second Network) output and threshold to see cytoplasmic output.");
    ...
    rename("LabKit_Cleaned"); waitForUser("Clean Up", "11. Remove small debris detected by LabKit.");
    run("Invert LUT"); waitForUser("Invert LUT", "12. Invert the coloring of the image for better viewing. Doesn't impact analysis.");
    ...
    run("Duplicate...", "title=Nuclei_Exp_2"); waitForUser("Create Maximum Limit Mask", "13. Expand nuclei to create limits for cells. We will use this to remove\nextraneous cytoplasms detected without nuclei.");
    imageCalculator("AND create", "LabKit_Cleaned", "Nuclei_Exp"); waitForUser("Combine with Cytoplasm", "14. AND Mask so we only have white areas that are present in both images.");
    run("Convert to Mask");
    rename("Cell_Territories_Grounded");
    run("Duplicate...", "title=Cell_Territories_Grounded_Aux"); waitForUser("Duplicate", "15. Duplicate for downstream steps.");
    ...
    setThreshold(1, 255); 
    run("Convert to Mask"); 
    run("Invert"); waitForUser("Separate Cells", "16. Voronoi to separate detected cells using nuclei.");
    imageCalculator("AND create", "LabKit_Cleaned", "Voronoi_Map"); waitForUser("AND Mask", "17. Combine with detected cytoplasms to get separated cells.");
    rename("Cell_Territories_2_A");
    imageCalculator("AND create", "Cell_Territories_2_A", "Nuclei_Exp_2"); waitForUser("Remove debris", "18. Combine with maximum mask again to remove debris. This will be used later in % positive calculation.");
    rename("Cell_Territories_2"); 

    // 7. Measurements 
    // We measure the Grounded mask (Total Area) to avoid under-counting pixels on the lines.
    selectWindow("SG_Channel");
    getVoxelSize(px_w, px_h, px_d, unit); waitForUser("Record Dimensions", "19. Select SG_Channel to record dimensions.");
    selectWindow("Cell_Territories_Grounded");
    setVoxelSize(px_w, px_h, px_d, unit);
    
    setThreshold(1, 255); waitForUser("Apply Dimensions & Measure", "20. Apply dimensions to mask image to ensure outputs are measured in microns, not pixels. Then, measure.");
...
selectWindow("SG_Channel_copy"); waitForUser("Make Copies", "21. Create numerous copies of SG_Channel to use later.");

// B. Preprocess to suppress aggregates
run("Subtract Background...", "rolling=1.75"); waitForUser("Subtract Background", "22. Improve SG detection.");
//THIS SETTING REDUCES BACKGROUND AND ENHANCES SG DETECTION.

run("Enhance Contrast", "saturated=0.095 normalize"); waitForUser("Enhance Contrast", "23. Improve SG detection.");
...
run("Auto Threshold", "method=MaxEntropy White"); waitForUser("Auto Threshold", "24. Use automatic thresholding to isolate SGs.");
run("Convert to Mask"); waitForUser("Convert to Mask", "25. Convert to mask so ImageJ can analyze it.");
...
rename("SG_Channel_and"); waitForUser("AND Mask with Cytoplasm", "26. Combine SG detection with cell boundaries so we only record SGs detected within cells.");
...
run("Analyze Particles...", "size=" + min_sg_size + "-" + max_sg_size + " add"); waitForUser("Find SGs", "27. Tell ImageJ to analyze particles that are between 0.2-10 microns^2.");
sg_count = roiManager("count"); waitForUser("Count SGs", "28. Count SGs that ImageJ just analyzed.");
	...
	waitForUser("Send to Results", "29. Send SG data to the results table.");
	// 4. Check how many results we have
	if (sg_count == 0) {
	    print("No particles detected (Results table empty).");
	} else {
		waitForUser("Measure SG Area", "30. Measure area for each SG");
...
if (sg_count > 0 && roiManager("count") == sg_count) {
    ...
    getPixelSize(unit, pw, ph); 
    pixelArea_um2 = pw * ph;
	waitForUser("Collect Dimensions", "31. Obtain dimensions from original SG_Channel.");
    ...
    setVoxelSize(pw, ph, 1, unit); waitForUser("Apply Dimensions", "32. Set dimensions from original SG_Channel so measurements are in microns, not pixels.");
    run("Set Measurements...", "area mean standard modal min centroid center perimeter bounding fit shape feret's integrated median skewness kurtosis area_fraction stack redirect=None decimal=3");
    run("Clear Results");
    roiManager("Deselect"); 
    roiManager("Measure"); waitForUser("Measure SGs", "33. Measure many variables for every SG."); 
    headings = newArray("Area", "Mean", "StdDev", "Mode", "Min", "Max", "X", "Y", "XM", "YM", "Perim.", "BX", "BY", "Width", "Height", "Major", "Minor", "Angle", "Circ.", "Feret", "IntDen", "Median", "Skew", "Kurt", "%Area", "RawIntDen", "Slice", "FeretX", "FeretY", "FeretAngle", "MinFeret", "AR", "Round", "Solidity");
...
waitForUser("Perform Calculations", "34. Macro performs many calculations in the background for the summary .csv file."); 
// -----------------------------------------------------------
...
selectWindow("Pct_Seeds"); waitForUser("New Image", "35. Set up new image for % SG Positive calculation."); 
...
nuclei_count_check = roiManager("count"); waitForUser("Recount Nuclei", "36. Recall nuclei ROIs stored in temporary zip file."); 
waitForUser("Add Nuclei", "37. Loop through nuclei to add them onto the new image from their exact center."); 
...
waitForUser("Voronoi", "38. Voronoi the nuclei to separate all nuclei and cells into distinct areas, so all cells are counted exactly once.\n% Positive Calculation is based on nuclei count, rather than cytoplasm count.");  
...
waitForUser("Paint Nuclei", "39. Loop through nuclei to paint them onto the new image from their exact center."); 
selectWindow("Pct_Shapes");
run("Duplicate...", "title=Pct_Limit");
run("Maximum...", "radius=75"); //waitForUser("Maximum from seed"); // Grow 100px from the Nucleus edge
waitForUser("Maximum Limit Mask", "40. Create limit from nuclei for SG % analysis.");
...
waitForUser("Separated Cell Limits", "41. Separate cell limits using the Voronoi mask generated previously, so cells aren't touching.");
...
setThreshold(1, 255);
run("Convert to Mask");
rename("Pct_Cellpose_Mask"); waitForUser("Recall Cell_Territories_Grounded", "42. Select Cell_Territories_Grounded which we created during cytoplasmic segmentation.");
imageCalculator("AND create", "Cell_Territories_V", "Pct_Cellpose_Mask");
rename("Cell_Territories_2_%"); waitForUser("AND Mask", "43. Create mask to combine cell territories that are separated via Voronoi mask and limited by maximum mask.");
...
run("Segment Image With Labkit", "input=SG_Channel_% segmenter_file=[" + classifier_path_SG_pct + "] use_gpu=false");
    ...
    if (isOpen("segmentation of SG_Channel_%")) {
        ...
        labkit_success = true;
        waitForUser("Maximum Limit Mask", "44. Let LabKit (Third Network) analyze the image to 'find' SGs. \n(Auto thresholding is too sensitive to classify cells as SG+/-, LabKit is too insensitive for counting/measuring SGs)");
...
if (labkit_success) {
    ...
    run("Convert to Mask"); waitForUser("Intensity Limit", "45. Create new limit to remove SGs that aren't bright enough (current min setting is 0, doesn't do anything).");
    
    // 2. Combine (remove granules that are too dim)
    imageCalculator("AND create", "Pct_Structure_Mask", "Pct_Intensity_Mask");
    rename("SG_Channel_Verified_PreFilter"); waitForUser("Remove Low Int SGs", "46. Combine intensity-limited SGs with LabKit output.");
    ...
    run("Invert LUT"); waitForUser("Size Limit", "47. Create new limit to remove SGs that are too small (current limit is 0.2 um^2, the smallest SGs we detect with auto thresholding).");
...
waitForUser("Select Voronoi Mask", "48. Select this mask to tell ImageJ where to look for SGs based on nuclei count.");
...
rename("SG_Bounded_Checking"); waitForUser("Combine with Cell_Territories_2", "49. AND mask so we only count SGs detected within cells.");
...
waitForUser("Loop Through Areas to Detect SGs", "50. Loop through ROIs (from Voronoi mask) to search for SGs. Only record those found within cell boundaries so % calculation aligns with nuclei count.");
// 4. Run the Loop
...
if (analyze_aux) {
    ...
    // 1. Safety Check: Do we have cells? Is the image open?
    if (nuclei_count > 0 && isOpen("Aux_Channel_Image")) {
        
        // 2. Define the Measurement Area (All Cellular Material)
        selectWindow("Cell_Territories_Grounded_Aux"); // The binary mask of cells
        setThreshold(1, 255);               
        run("Create Selection");            // Create one giant ROI of all cells
        waitForUser("Analyze Aux Protein", "51. Recall cell boundaries to measure intensity of aux protein within cells.");
        // 3. Measure the Signal
        selectWindow("Aux_Channel_Image"); waitForUser("Recall Aux Protein Channel", "52. Recall aux protein for intensity analysis.");
        run("Restore Selection"); // Apply the cell mask to the Aux image
        waitForUser("Restore Cell ROIs", "53. Restore cell boundaries that we just collected from Cell_Territories_Grounded_Aux so we only measure intensity inside these ROIs.");
        // 'getStatistics' is faster than 'Measure' for whole-image stats
        getStatistics(area, mean, min, max, std, histogram);  
        waitForUser("Measure", "54. Measure intensity.");
        total_signal_in_all_cells = area * mean; // Equivalent to RawIntDen
        
        // 4. Calculate Per-Cell Averages
        aux_avg_intden = total_signal_in_all_cells / nuclei_count;
        aux_avg_mean   = mean; // Average brightness of the cellular monolayer
        run("Select None"); // Clean up ROI
...
run("Collect Garbage"); 
setBatchMode(false);
waitForUser("Print, Save, Reset", "55. Print results. Then close all images and clear the results and ROI manager. Set batch mode = false to prep for next image.");
}
